...
export const Route = createFileRoute("/resellers")({
  head: () => ({ meta: [{ title: "Resellers — Kstomer" }] }),
  component: Resellers,
});
...
function Resellers() {
  return (
    <AppShell
      ...
      search={{ placeholder: "Rechercher un partenaire…" }}
    >
      <div className="grid grid-cols-1 md:grid-cols-3 gap-5 mb-8">
        <Kpi label="Partenaires actifs" value="14" icon={<Store className="h-4 w-4" />} />
        <Kpi label="CA partenaires (M)" value="34 400 €" icon={<TrendingUp className="h-4 w-4" />} />
        <Kpi label="Tier Gold" value="3" icon={<Award className="h-4 w-4" />} />
      </div>
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="text-[11px] uppercase tracking-wider text-muted-foreground border-b border-border bg-muted/40">
              ...
              <th className="text-left p-4">Revenu</th>
              <th className="text-left p-4">Santé</th>
            </tr>
          </thead>
          <tbody>
            {RESELLERS.map((r) => (
